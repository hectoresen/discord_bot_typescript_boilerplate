...
const timeoutCommand: ICommand = {
  data: new SlashCommandBuilder()
    .setName('timeout')
    .setDescription(
      'Mute a user for a specified amount of time (Administrators or moderators only)'
    )
  ...
  run: async (client: ExtendedClient, interaction: CommandInteraction) => {
    ...
    try {
      ...
      new ConsoleStatusHandler(
        '⏳',
        `The user ${userDisplayName} has been muted for ${duration} minutes by ${interaction.user.globalName}`,
        'ok'
      )
    } catch (error) {
      ...
      await interaction.reply({
        content:
          'An error occurred while muting the user, possibly due to higher role permissions.',
        ephemeral: true,
      })
    }
  },
