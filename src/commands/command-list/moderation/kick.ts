...
const kickCommand: ICommand = {
  data: new SlashCommandBuilder()
    .setName('kick')
    .setDescription(
      'Kick a user from the server (Administrators or moderators only)'
    )
  ...
  run: async (client: ExtendedClient, interaction: CommandInteraction) => {
    ...
    if (
      !member ||
      (!member.roles.cache.has(roles.adminRoleId) &&
        !member.roles.cache.has(roles.moderatorRoleId))
    ) {
    ...
    try {
      ...
      const userDisplayName =
        userTarget.user.globalName || userTarget.user.username || 'Unknown User'
      ...
      new ConsoleStatusHandler(
        '⚔️',
        `The user ${userDisplayName} has been kicked by the administrator ${interaction.user.globalName}`,
        'ok'
      )
    } catch (error) {
      ...
      await interaction.reply({
        content:
          'An error occurred while kicking out the user, probably because they have a higher role than the command executor',
        ephemeral: true,
      })
    }
  },
