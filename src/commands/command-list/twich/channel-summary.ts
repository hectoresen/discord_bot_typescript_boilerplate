...
const channelSummary: ICommand = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      'Shows a brief summary of the Twitch channel for the last 30 days.'
    )
  ...
  run: async (client: ExtendedClient, interaction: CommandInteraction) => {
    ...
    if (!channelName.value) {
      return interaction.reply({
        content: 'Please provide a valid Twitch channel name.',
        ephemeral: true,
      })
    }
    ...
    try {
      const response = await axios.get(
        `https://twitchtracker.com/api/channels/summary/${channelName.value}`
      )
      const data = response.data

      if (Object.entries(data).length === 0) {
        return await interaction.reply({
          content: `No channel found with the name: ${channelName.value}`,
          ephemeral: true,
        })
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: 'Rank', value: data.rank.toString(), inline: true },
          {
            ...
            value: data.minutes_streamed.toString(),
            inline: true,
          },
          {
            name: 'Average Viewers',
            value: data.avg_viewers.toString(),
            inline: true,
          },
          {
            name: 'Max Viewers',
            value: data.max_viewers.toString(),
            inline: true,
          },
          {
            name: 'Hours Watched',
            value: data.hours_watched.toString(),
            inline: true,
          },
          { name: 'Followers', value: data.followers.toString(), inline: true },
          {
            name: 'Total Followers',
            value: data.followers_total.toString(),
            inline: true,
          },
          {
            name: 'Powered by',
            value: '[TwitchTracker.com](https://twitchtracker.com)',
            inline: false,
          }
        )
        .setFooter({
      ...
      new ConsoleStatusHandler(
        '💽',
        `Error getting data from a twitch channel - ${error}`,
        'error'
      )
      await interaction.reply({
        content: 'An error occurred while fetching the channel summary.',
        ephemeral: true,
      })
    }
  },
