...
export const getFilesByDir = async (
    ...
    events: [
      {
        ...
        route: resolve(__dirname, '../src/events', folderName),
      },
      {
        enviroment: 'production',
        route: resolve(__dirname, '../dist/events', folderName),
      },
    ],
    commands: [
      {
        enviroment: 'development',
        route: resolve(__dirname, '../src/commands', folderName),
      },
      {
        enviroment: 'production',
        route: resolve(__dirname, '../dist/commands', folderName),
      },
    ],
  }
  ...
  if (!paths) {
    new ConsoleStatusHandler(
      '💽',
      'No valid handler specified, must be commands or events',
      'error'
    )
    return
  }
  ...
  const getFilePath = () => {
    ...
    const resolvePath = (enviroment: string) => {
      const enviromentPath = paths.find(
        (path) => path.enviroment === enviroment
      )
  ...
  for (const filePath of allFiles) {
    try {
      ...
      new ConsoleStatusHandler(
        '💽',
        `There was an error loading ${filePath} file - ${error}`,
        'error'
      )
    }
  }
